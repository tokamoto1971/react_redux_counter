...
describe('async actions', () => {
  ...
  it('{type: INCREMENT, payload: 123}とか約1000ms後に返されるはず', (done) => {
    ...
    .then(() => {
      ...
      expect(actions).toEqual([{ type: INCREMENT, payload: 123 }, { type: INCREMENT, payload: 456 }])
    })
    .then(done)
    .catch(done)
  })
  ...
  it('{type: DECREMENT, payload: 456}とか約1000ms後に返されるはず', (done) => {
    ...
    .then(() => {
      const actions = store.getActions()
      expect(actions).toEqual([{ type: DECREMENT, payload: 123 }, { type: DECREMENT, payload: 456 }])
    })
    .then(done)
    .catch(done)
  })
